...
<div class="flex flex-wrap justify-between">
  <form method="GET" action="{{url('/teacher/feed/filter')}}" role="search" enctype="multipart/form-data">
  ...
  <div>
    ...
    <ul class="list-reset leading-loose py-2">
      <li class="py-1"><a href="{{url('/teacher/feeds?search=achievement')}}">Achivements / Kudos</a></li>
      <li class="py-1"><a href="{{url('/teacher/feeds?search=birthday')}}" value="birthday">Birthdays</a></li>
      <li class="py-1"><a href="{{url('/teacher/feeds?search=workanniversary')}}" value="workanniversary">Work Anniversaries</a></li>
      <li class="py-1"><a href="{{url('/teacher/feeds?search=teacher')}}" value="teacher">By Teacher / Staff</a></li>
      <li class="py-1"><a href="{{url('/teacher/feeds?search=student')}}" value="student">By Students</a></li>
    </ul>
  </div>
  <div class="py-3">
    <p class="font-semibold text-red-600">Filter By Source</p>
    <ul class="list-reset leading-loose py-2">
      <li class="py-1"><a href="{{url('/teacher/feeds?list=school')}}" value="school">School</a></li>
      <li class="py-1"><a href="{{url('/teacher/feeds?list=all_class')}}" value="all_class">All Classes</a></li>
      <li class="py-1"><a href="{{url('/teacher/feeds?list=select_class')}}" value="select_class">My Class Only</a></li>
    </ul>
  </div>
  </div>
    </form>
                    ...
                    <!-- feed template -->
                        @foreach($feeds as $feed)
                            <div class="border-b-2 mb-4">
                                ...
                                <div> 
                                    <show-feed url="{{ url('/') }}" id="{{ $feed->id }}" mode="teacher" auth_id="{{ \Auth::id() }}"></show-feed> 
                                </div>  
                            </div>
                        @endforeach
                    <!-- feed template -->
                    ...
                    <ul class="flex flex-wrap text-sm">
                        @foreach($tags as $tag)
                            @if($tag->tag_name!=NULL)
                                <li class="px-2 py-1 border mr-1 my-1"><a href="{{url('/teacher/feeds?search='.$tag->tag_name)}}">{{$tag->tag_name}}</a>({{$tag->getTag($tag->tag_name)}})</li>
                            @endif
                        @endforeach
                    </ul>
                </div>
